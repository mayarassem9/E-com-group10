const customerServiceMessages = [
  {
    ...
    message: "I have a problem in",
    isRead: false,
    date: "Jan 20, 2024, 9:24 PM GMT+2",
  },
  {
    id: 2,
    userId: 5,
...
function loadAllMessages() {
  ...
  allMessages.forEach((message) => {
    const row = `<tr${message.isRead ? ' class="read-message"' : ""} >
                        ...
                        <td>${message.date}</td>
                        <td>
                            <button class="btn btn-danger rounded-circle" onclick="deleteMessage(${
                              message.id
                            })">
                                <i class="fa-solid fa-trash" style="color: #ffffff;"></i>
                            </button>
                            <button class="btn  rounded-circle " data-bs-toggle="modal" data-bs-target="#readMessageModal" onclick="markAsRead(${
                              message.id
                            })">
  ...
  mycustomerServiceMessages.forEach((message, index) => {
    if (message.id === id) {
      isDeleteConfirmed().then((confirmed) => {
        if (confirmed) {
          mycustomerServiceMessages.splice(index, 1);
          saveMessagesToLocalStorage(mycustomerServiceMessages);
          loadAllMessages();
        }
      });
      // Break out of the loop after deleting the message
      return;
    }
  });
}
